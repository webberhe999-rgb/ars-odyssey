...
public final class TargetInputResolver {
    private TargetInputResolver() {
    }

    public static SearchIntent resolveIntent(String query) {
    ...
    public static TargetQuery resolveTargetQuery(String query) {
        return TargetQueryParser.parse(query);
    }

    public static Optional<TargetDescriptor> resolveTargetDescriptor(String query) {
        return targetDescriptorOf(resolveIntent(query));
    }

    public static Optional<TargetDescriptor> targetDescriptorOf(SearchIntent intent) {
        if (intent == null || intent.resolvedId() == null) {
            return Optional.empty();
        }

        return switch (intent.type()) {
            case ENTITY -> Optional.of(TargetDescriptor.entityType(intent.resolvedId()));
            case BLOCK -> Optional.of(TargetDescriptor.block(intent.resolvedId()));
            case ITEM -> Optional.of(TargetDescriptor.item(intent.resolvedId()));
            case TAG -> Optional.of(TargetDescriptor.unknown(intent.resolvedId().toString()));
